...
module.exports = {
  ...
  run: async (client,interaction,args) => {
    ...
    if(role?.managed) return  interaction.reply(`🙄 - **${role.name}** is managed by integration and can't be given.`);
    if(interaction.guild.members.me.roles.highest.position <= role?.position) return interaction.reply(`🙄 - I couldn't change the roles for that user. Please check my permissions and role position.`);
    if (interaction.member.roles.highest.position <= role?.position && interaction.guild.ownerId !== interaction.member.id) return interaction.reply(`🙄 - **${role.name}**'s position is higher than yours.`);
    ...
    if(typesubcommand === "give"){
      ...
      target.roles.add(role.id).then(() =>{
        ...
        return interaction.reply(`🙄 - I can't find the role **${role.name}**.`);
      });
      ...
      let embed = new MessageEmbed().setColor("#DC143C");
