...
module.exports = {
  ...
  run: async (client, interaction, args, guildData) => {
    try {
      ...
      if (target.roles.highest.position >= interaction.member.roles.highest.position && interaction.guild.ownerId !== target.id && interaction.guild.ownerId !== interaction.member.id || interaction.guild.ownerId == target.id) {
      ...
      target?.roles.add(role?.id).then(c => {
        setLongTimeout(async () => {
          ...
          if (!newobj) return;
          target.roles.remove(newobj.roleID).catch(err => console.error(err));
