  ...
  options: [
    {
      name: "add",
      type: 1,
      description: `Add roles for [${types.map(type => type.name).join("/")}]!`,
      options: [
        {
          name: "type",
    ...
    },
    {
      name: "remove",
      type: 1,
      description: `Remove roles from [${types.map(type => type.name).join("/")}]!`,
      options: [
        {
          name: "type",
  ...
  run: async (client, interaction, args, guildData) => {
    ...
    if (valid?.length) {
      return interaction.reply({
        content: `**🙄 - I can't save admin roles.**`
      });
    }
