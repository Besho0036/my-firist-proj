...
module.exports = {
  ...
  run: async (client, interaction, args, guildData) => {
    ...
    let filter = response => response.content.toLowerCase().trim().length === c.length;
    interaction.reply({ files: [attachment] }).then(() => {
      ...
      interaction.channel.awaitMessages({ filter, max: 1, time: 15000, errors: ['time'] }).then(async (collected) => {
        ...
        interaction.followUp({ content: `> **مبروك! الفائز هو ${author} ، قام بكتابة الكلمة بشكل صحيح قبل انتهاء الوقت بـ ${time.toFixed(2)} ثانية!**` });
